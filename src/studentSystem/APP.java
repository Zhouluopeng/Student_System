...
public class APP {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            System.out.println("请选择操作 1 登录 2 注册 3忘记密码 4 退出");
    ...
    private static void forgetPassword(ArrayList<User> list) {
        ...
        if (!(user.getPersonID().equalsIgnoreCase(personID) && user.getPhoneNumber().equals(phoneNumber))) {
            System.out.println("身份证号码或手机号码输入错误，不能修改密码");
            return;
        }
        String password;
        while (true) {
    ...
    private static void register(ArrayList<User> list) {
        ...
        while (true) {
            ...
            //把用户信息添加到集合当中
            if (flag1) {
                System.out.println("格式满足条件，继续判断用户唯一性");
                continue;
            }
    ...
    private static void printList(ArrayList<User> list) {
        for (int i = 0; i < list.size(); i++) {
            ...
            System.out.println(user.getUsername() + "," + user.getPassword() +
                    "," + user.getPhoneNumber() + "," + user.getPersonID());
        }
    }
    ...
    private static void login(ArrayList<User> list) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            while (true) {
                String rigthcode = getCode();
                System.out.println("当前验证码为" + rigthcode);
                System.out.println("请输入验证码");
                String code = sc.next();
                if (code.equalsIgnoreCase(rigthcode)) {
            ...
            if (result) {
                ...
                StudentSystem.startStudetnSystem();
                break;
            } else {
                System.out.println("登陆失败，用户名或者密码错误");
                if (i == 2) {
                    System.out.println("当前帐号" + username + "被锁定，请联系客服");
                    return;
                } else {
